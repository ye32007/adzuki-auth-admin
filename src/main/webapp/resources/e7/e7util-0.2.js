...
/**
 * 初始化checkbox控件选项
 * @param name              控件名称(html元素的name属性对应的名称)
 * @param items             选项列表:[{'id':'1', 'text':'北京'}, {'id':'2', 'text':'上海'}]
 * @param defaultChecked    默认选项数组: ['1', '2']
 */
E7.initCheckbox = function (name, items, defaultChecked) {
    $('input:checkbox[name="' + name + '"]').each(function (idx, elem) {
        $('label[for="' + $(elem).attr('id') + '"]').val(items[idx].text);
        $(elem).val(items[idx].id);
        if (defaultChecked && defaultChecked.includes(items[idx].id)) {
...
E7.ajaxDefault={
	...
	completeCallback: null,	//完成后的回调函数
	...
	complete : function(data){
		...
		if(this.completeCallback != null) this.completeCallback(data.responseJSON);
	},
...
var $iframe = $('<iframe id="modelFrame" name="modelFrame" width="100%" src="" border="0" frameborder="0" scrolling="no">'
